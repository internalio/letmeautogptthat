# Modified Auto-GPT: An Autonomous GPT-4 Experiment single run with database

A modified Auto-GPT that takes in a task from a database, attempts to process it, and output it.
...
Uses the `requirements-internal.txt` file and the `auto.py` file.

CD into this project's directory and run:

```bash
docker build -f Dockerfile-Internal -t autogpt:0.0.1 .
```
...
autogpt:0.0.1 <job_id : in supabase>
```

## Changes from original

- Less commands available to agent
- Pass along thoughts to a database to record (uses Supabase)
- Query task from database
- New command `write_output`. Code to detect this and record it to database. See `CustomAgent` in `auto.py`

The README below is directly from [Auto-GPT](https://github.com/Significant-Gravitas/Auto-GPT) release 0.3.0
